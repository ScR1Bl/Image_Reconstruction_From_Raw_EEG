"""Train the high-capacity multi-layer DINO color oracle v2."""
...
def arguments() -> argparse.Namespace:
    ...
    p.add_argument("--output", default="runs/dino_color_oracle_v2")
    p.add_argument("--baseline-bank", default="data/derived/visual_targets_dinov2s_192.pt")
    p.add_argument("--baseline-decoder", default="runs/dino_color_oracle_v1/best.pt")
...
def save_panel(model, dataset, legacy, device, path, count):
    ...
    headers = ("target", "oracle v1", "v2 soft", "v2 token argmax", "learned palette")
    tile, label = 128, 22
